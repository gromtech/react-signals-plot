...
class DefaultZoomer extends Zoomer {
  ...
  getZoomedByTouches(extent, params) {
    ...
    if (extent && params && Array.isArray(params.shifts)) {
      ...
      if (shifts.length === 2) {
        zoomed = {
          x: this.getAxisBounds(extent.x[0], extent.x[1], shifts[0].x1, shifts[0].x2, shifts[1].x1, shifts[1].x2, 8),
          y: this.getAxisBounds(extent.y[0], extent.y[1], shifts[0].y1, shifts[0].y2, shifts[1].y1, shifts[1].y2, 8),
        };
      }
    }
    return zoomed;
